...
const TextEditor = ({
  ...
  const { token }: any = useContext(FetchContext);
  ...
  const getAllImages = async () => {
    const request = await fetch(
      `${process.env.NEXT_PUBLIC_BACK_END_URL}/api/v1/webpanel/media/${editor.images.getPath}`,
      {
        headers: {
  ...
  const setCodeStateHandler = () => {
    ...
    if (editorBody?.querySelector(".editor-body")) {
      ...
      makeElement.innerHTML =
        //@ts-ignore
        editorBody.querySelector(".editor-body").innerHTML;
      makeElement
  ...
  const imgModal = (e: any) => {
    ...
    if (img) {
      ...
      if (title) setTitle(title.innerText);
      else setTitle("");
  ...
  const handleSetSelect = (e: any) => {
    ...
    if (current.classList.contains("select-row")) {
      ...
      countEl.setAttribute(
        "class",
        "absolute p-2 bg-indigo-500 rounded-xl w-7 h-7 text-white count text-sm flex justify-center items-center -mt-3 ml-2"
      );
    ...
    Array.from(
      e.target.closest(".bg-stripes-pink").querySelectorAll(".select-row")
    ).map((v, k) => {
      //@ts-ignore
      if (v.getAttribute("data-content")) {
        //@ts-ignore
        selectedRow[v.getAttribute("data-select")] = JSON.parse(
          //@ts-ignore
          v?.getAttribute("data-content")
        );
      }
    });
    setRow(selectedRow);
  ...
  const insertImg = (el: any) => {
    const preview = el.currentTarget
      .closest(".modal-content")
  ...
  const saveSourceCode = (el: any) => {
    ...
    Array.from(remark?.children).map((v: any, k: any) => {
      if (k > 0) v.remove();
    });
    //@ts-ignore
    Array.from(dom.querySelector("body")?.children).map((v: any) => {
      remark.append(v);
    });
  ...
  const upload = async (e: any) => {
    const setSelectedImage = document
    ...
    if (setSelectedImage) {
      ...
      if (files.length > 0) {
        ...
        const request = await fetch(
          `${process.env.NEXT_PUBLIC_BACK_END_URL}/api/v1/webpanel/media/${editor.images.uploadPath}`,
          {
            method: "POST",
            headers: {
              authorization: `Bearer ${localStorage.getItem("accessToken")}`,
  ...
  const removeImage = async (e: any) => {
    if (confirm("Are you sure you want to remove this image?") === true) {
      let setSelectedImage = e.currentTarget
        .closest(".modal-content")
        ?.querySelectorAll(".image-select");
      ...
      if (removeAmount > 0) {
        ...
        const request = await fetch(
          `${process.env.NEXT_PUBLIC_BACK_END_URL}/api/v1/webpanel/media`,
          {
            method: "DELETE",
            headers: {
  ...
  const UnderOrderList = (style: any) => {
    ...
    document.execCommand(
      "insertHTML",
      false,
      `<ul class="${style} ml-6"><li>${selection}</li></ul>`
    );
    setTimeout(() => {
      setUlVisible(false);
    }, 100);
  ...
  const OrderList = (style: any) => {
    ...
    document.execCommand(
      "insertHTML",
      false,
      `<ol class="${style} ml-6"><li>${selection}</li></ol>`
    );
    setTimeout(() => {
      setUlVisible(false);
    }, 100);
  ...
  const FontSize = (size: any) => {
    ...
    document.execCommand(
      "insertHTML",
      false,
      `<p style="font-size:${size}; line-height:normal">${selection}</p>`
    );
  };
  // 15.
  const Heading = (select: String) => {
    ...
    switch (select) {
      ...
      case "h6":
        ...
        break;
      case "code":
        className = "text-sm text-slate-500 dark:text-slate-400 whitespace-pre";
        break;
      default:
        break;
    }
  ...
  function addLink() {
    ...
    document.execCommand(
      "insertHTML",
      false,
      '<a class="link" href="' + linkURL + '" target="_blank">' + sText + "</a>"
    );
  }
  // 22. Full screen
  const fullScreenMode = () =>
    document.getElementById(EditorId)?.classList.toggle("full-screen-mode");
  //================= end: Exec Command ==================//
  //======================================================//
  ...
  const sortHandler = (e: any) => {
    ...
    editor.querySelectorAll(".drag-sort-enable").forEach((el: any) => {
      ...
      el
        ...
        ?.setAttribute("contenteditable", edit);
    });
  ...
  const HandleDrop = (item: any) =>
    item.target.closest(".grid").classList.remove("focus");
  //@ts-ignore
                ...
                <div className="group flex border-r px-1 border-slate-300">
                  ...
                  <div className="tool-item relative flex items-center border border-transparent hover:border hover:border-slate-200 cursor-pointer rounded">
                    ...
                    <button
                      ...
                      style={{ height: "32px" }}
                      onMouseOver={() => setFcVisible(true)}
                      onMouseOut={() => setFcVisible(false)}
                    >
                      ...
                      >
                        <div className="p-4 bg-white">
                          ...
                          <HexColorInput
                            color={color}
                            onChange={setColorHandler}
                            className="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1 mt-2 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                          />
                        </div>
                      </div>
                    </button>
                ...
                <div className="group flex border-r px-1 border-slate-300">
                  <div className="tool-item relative flex items-center border border-transparent hover:border hover:border-slate-200 cursor-pointer rounded">
                    ...
                    <button
                      ...
                      className="pointer bg-white hover:bg-slate-200 max-h[32] flex items-center"
                      style={{ height: "32px" }}
                      onMouseOver={() => setUlVisible(true)}
                      onMouseOut={() => setUlVisible(false)}
                    >
                      <RxCaretDown />
                      <div
                        id="dropdownDivider"
                      ...
                      >
                        <ul>
                          <li>
                            <button
                              type="button"
                              className="block px-4 py-1 text-[14px] hover:bg-slate-100"
                              onClick={() => {
                                UnderOrderList("list-disc");
                              }}
                            >
                  ...
                  <div className="tool-item relative flex items-center border border-transparent hover:border hover:border-slate-200 cursor-pointer rounded">
                    ...
                    <button
                      ...
                      className="pointer bg-white hover:bg-slate-200 max-h[32] flex items-center"
                      style={{ height: "32px" }}
                      onMouseOver={() => setOlVisible(true)}
                      onMouseOut={() => setOlVisible(false)}
                    >
                      <RxCaretDown />
                      <div
                        ...
                        <ul>
                          <li>
                            <button
                              type="button"
                              className="block px-4 py-1 text-[14px] hover:bg-slate-100"
                              onClick={() => OrderList("list-decimal")}
                            >
                              Default
                            </button>
                          </li>
                          <li>
                            <button
                              type="button"
                              className="block px-4 py-1 text-[14px] hover:bg-slate-100"
                              onClick={() => OrderList("list-[lower-alpha]")}
                            >
                              Lower Alpha
                            </button>
                          </li>
                          <li>
                            <button
                              type="button"
                              className="block px-4 py-1 text-[14px] hover:bg-slate-100"
                              onClick={() => OrderList("list-[lower-greek]")}
                            >
                              Lower Greek
                            </button>
                          </li>
                          <li>
                            <button
                              type="button"
                              className="block px-4 py-1 text-[14px] hover:bg-slate-100"
                              onClick={() => OrderList("list-[lower-roman]")}
                            >
                              Lower Roman
                            </button>
                          </li>
                          <li>
                            <button
                              type="button"
                              className="block px-4 py-1 text-[14px] hover:bg-slate-100"
                              onClick={() => OrderList("list-[upper-alpha]")}
                            >
                              Upper Alpha
                            </button>
                          </li>
                          <li>
                            <button
                              type="button"
                              className="block px-4 py-1 text-[14px] hover:bg-slate-100"
                              onClick={() => OrderList("list-[upper-roman]")}
                            >
                              Upper Roman
                            </button>
                          </li>
                ...
                <div className="group flex border-r px-1 border-slate-300">
                  <div className="tool-item relative flex items-center border border-transparent hover:border hover:border-slate-200 cursor-pointer rounded">
                    ...
                    <button
                      ...
                      className="pointer bg-white hover:bg-slate-200 max-h[32] flex items-center"
                      style={{ height: "32px" }}
                      onMouseOver={() => setFontSizeVisible(true)}
                      onMouseOut={() => setFontSizeVisible(false)}
                    >
                      <RxCaretDown />
                      <div
                  ...
                  <div className="tool-item relative flex items-center border border-transparent hover:border hover:border-slate-200 cursor-pointer rounded">
                    ...
                    <button
                      ...
                      className="pointer bg-white hover:bg-slate-200 max-h[32] flex items-center"
                      style={{ height: "32px" }}
                      onMouseOver={() => setHeadingVisible(true)}
                      onMouseOut={() => setHeadingVisible(false)}
                    >
                      <RxCaretDown />
                      <div>
                        <ul
                  ...
                  <div className="tool-item relative flex items-center border border-transparent hover:border hover:border-slate-200 cursor-pointer rounded">
                    ...
                    <button
                      ...
                      className="pointer bg-white hover:bg-slate-200 max-h[32] flex items-center"
                      style={{ height: "32px" }}
                      onMouseOver={() => setLineHeight(true)}
                      onMouseOut={() => setLineHeight(false)}
                    >
                      <RxCaretDown />
                      <div
