...
export default function ModalDialog({
  ...
  const rows = [
    ...
    [
      { col: "col-span-4", content: "image" },
      { col: "col-span-8", content: "text" },
    ],
    [
      { col: "col-span-4", content: "image" },
      { col: "col-span-4", content: "image" },
        ...
        <ModalContent>
          {(onClose) => (
            <>
              ...
              <ModalBody>
                <div
                  className="bg-stripes-pink p-2"
                  style={{
                    borderRadius: "10px",
                    backgroundColor: "#9772f41a",
                    backgroundImage:
                      "linear-gradient(135deg, #6148ec80 10%, #0000 0, #0000 50%, #6148ec80 0, #6148ec80 60%, #0000 0, #0000",
                ...
                >
                  {Array.from(rows).map((v, k) => (
                    <div
                      key={k}
                      className={`grid grid-cols-1 md:grid-cols-12 gap-4 ${k > 0 ? `mt-4` : ``} transition-all cursor-pointer rounded-lg hover:ring hover:ring-indigo-500`}
